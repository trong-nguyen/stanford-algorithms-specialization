...
def select_median_of_medians(array):
	def _select(array):
		# number of sampling points, according to statistic analysis, this results in a 30-70 or better split
		K = 5
		def mid_index(array):
		...
		# recursion
		sub = [mid_point(array[K*i:min(K*(i+1),n)]) for i in range(n/K)]
		return _select(sub)
...
def test_assignment():
	...
	print '{} comparisons with random item as pivot'.format(comparisons_in_quick_sort(array, select_random_as_pivot))
	print '{} comparisons with median of medians as pivot'.format(comparisons_in_quick_sort(array, select_median_of_medians_as_pivot))
